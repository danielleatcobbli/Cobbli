...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let colors = Array.isArray(parsed.colors)
      ? parsed.colors.filter((c) => COLORS.includes(c)).slice(0, 4)
      : [];
    if (colors.length >= 4) colors = ["Multi"];
